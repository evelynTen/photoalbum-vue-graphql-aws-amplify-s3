...
function useMachine(stateMachine, options) {
    var service = fsm_1.interpret(fsm_1.createMachine(stateMachine.config, options ? options : stateMachine._options)).start();
    var state = vue_1.shallowRef(getServiceValue(service));
    vue_1.onMounted(function () {
        service.subscribe(function (currentState) { return (state.value = currentState); });
    });
    vue_1.onBeforeUnmount(service.stop);
    return { state: state, send: service.send, service: service };
}
exports.useMachine = useMachine;
function useService(service) {
    var serviceRef = vue_1.isRef(service)
        ? service
        : vue_1.shallowRef(service);
    var state = vue_1.shallowRef(serviceRef.value.state);
    vue_1.watch(serviceRef, function (service, _, onCleanup) {
